...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final String samplesPath = "samples";
        try {
            String[] questionnairePaths = getAssets().list(samplesPath);
            for(final String eaQuestionnairePath:questionnairePaths) {
                Button btn = new Button(this);
                btn.setText(eaQuestionnairePath);
                btn.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        Intent i_survey = new Intent(MainActivity.this, SurveyActivity.class);
                        i_survey.putExtra("json_survey", loadSurveyJson(samplesPath + "/" + eaQuestionnairePath));
